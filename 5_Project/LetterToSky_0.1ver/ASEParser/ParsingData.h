...
namespace ASEParser
{
	...
	public:
		...
		/// *NODE_NAME
		///----------------------------------
		std::string	m_nodename;		// 어떤 식인지는 모르지만 아래의 m_meshname과 같은 이름이 한번 더 들어간다, 아마도 메쉬의 이름 리스트라던지...
		std::string	m_nodeparent;	// 부모 노드의 이름, 이것이 있다면 부모가 있는 것이다
		bool m_isparentexist;		// 그래도 확실히 bool형으로 가지고 있자
		Mesh* m_parent;				// 부모를 변수로 가지고 있자
		int m_parentMeshCount;		// 부모로 존재하는 Mesh가 총 몇개인지?
